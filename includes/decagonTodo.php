...
class decagonTodo {
	...
	public function __construct(){
		...
		add_filter( "manage_decagon_todo_posts_columns", array($this, 'change_columns'));
		...
		add_action('wp_ajax_nopriv_decagon_ajax', array($this,'ajax'));
		add_action('wp_ajax_decagon_ajax', array($this,'ajax'));
	}
	...
	public function init(){
		...
		if( preg_match('/\/todo\/?$/',$_SERVER['REQUEST_URI'])){
			$base_url = plugins_url( 'app/' , DECAGON_TODO_FILE);
			require DECAGON_TODO_PATH.'/app/index.php';
			exit;
		}
	...
	private function add_post_type(){
		...
		register_post_type( 'decagon_todo',
			array(
				'labels' => array(
					'name' => __( 'Todo items' ),
